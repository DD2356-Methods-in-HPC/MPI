...
void distribute_blocks(double* A, double* B, double* local_A, double* local_B, int matrix_size, int rank, int processes, int block_size, MPI_Comm grid_comm) {
    ...
    MPI_Type_vector(block_size * block_size, 1, matrix_size / 2, MPI_DOUBLE, &block_type);
    ...
    if (rank == 0) {
        ...
        for (int i = 0; i < processes; i++) {
            ...
            displacements[i] = coords[0] + coords[1]; //(coords[0] * block_size * matrix_size) + (coords[1] * block_size * block_size); 
        }
